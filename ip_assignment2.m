...
FFT_curr_image = find_DFT(im2double(current_image));
...
FFT_blur_kernel_1 = find_DFT(blur_kernel_1); %Computing FFT of blur kernel
...
current_image = real(find_IDFT(F_cap));
cla; %remove the previous image
axes(handles.axes1); % handle to axes
imshow(current_image); %display image
...
laplace_fft = find_DFT(laplace_padded); %computing DFT of the laplace matrix
...
for u=1:M
    for v=1:N
        ...
        for x=1:M
            for y=1:N
                output(u,v)=output(u,v)+image(x,y).*exp((-1j).*2.*pi.*((x-1).*(u-1)./M)+(y-1).*(v-1)./N);
            end
        end
    end 
end
